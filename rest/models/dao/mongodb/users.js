...
exports.retriveUsers = function (options) {
  return new Promise(function (resolve, reject) {
...
exports.authorizeUser = function (username, password) {
  return new Promise(function (resolve, reject) {
    exports.retrieveUserByUsername(username).then(function (row) {
      appLogger.log(row, 'warn')
      if (row) {
        var passwordHash = Buffer.from(row.passwordHash, 'hex') // Combined hash, salt
        crypto.verifyPassword(password, passwordHash, function (err, verified) {
          if (err) {
            ...
            appLogger.log('Password no match', 'error')
            resolve(null)
          }
          else {
            appLogger.log('Logged in', 'warn')
            // Logged in.
            resolve(row)
          }
        })
        ...
        // User not found.
        appLogger.log('User not found', 'warn')
        resolve(null)
      }
    })
      .catch(function (err) {
...
exports.retrieveUserProfile = function (username) {
  return new Promise(function (resolve, reject) {
    ...
    db.selectOne(sql, username, function (err, row) {
      if (err) {
        reject(err)
      }
      else {
...
exports.getRoles = function () {
  return new Promise(function (resolve, reject) {
    db.fetchRecords('userRoles', {}, function (err, results) {
      if (err) reject(err)
