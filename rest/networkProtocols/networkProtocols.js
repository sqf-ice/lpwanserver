...
NetworkProtocolAccess.prototype.getProtocol = function( network ) {
    ...
    return new Promise( async function( resolve, reject ) {
        ...
        if ( ! networkProtocolMap[ id ] ) {
            try {
                appLogger.log(JSON.stringify(JSON.stringify(me.npAPI)));
                // We'll need the protocol for the network.
                // var np = await me.npAPI.networkProtocols.retrieveNetworkProtocol( network.networkProtocolId );
                var np = await me.npAPI.networkProtocols.prototype.retrieveNetworkProtocols(  );
...
NetworkProtocolAccess.prototype.pullCompanies = function( dataAPI, network ) {
    ...
    return new Promise( async function( resolve, reject ) {
        ...
        // up or is expired)
        me.sessionWrapper( network, loginData, function( proto, sessionData ) {
            return proto.api.pullCompanies( sessionData,
                network,
                dataAPI );
        })
            .then( function( ret ) { resolve( ret ); } )
...
// or creates the remote company.  This may or may not do as promised (haha) -
// the implementation is completely up to the developers of the protocols.
NetworkProtocolAccess.prototype.addProtocolDataForCompany = function( dataAPI, network, remoteOrganization, localCompany ) {
    var me = this;
    return new Promise( async function( resolve, reject ) {
        // Get the protocol for the network.
        var netProto = await me.getProtocol( network );
        ...
        // up or is expired)
        me.sessionWrapper( network, loginData, function( proto, sessionData ) {
            return proto.api.addProtocolDataForCompany( sessionData,
                network,
                remoteOrganization,
                localCompany,
                dataAPI );
